...
for row in image2:
    ...
    converted_row[1], converted_row[2] = converted_row[2], converted_row[1]
    converted_image.append(converted_row.reshape(1,3))

converted_image = cv.pyrDown(converted_image)
converted_image = cv.pyrUp(converted_image)
converted_image = np.asarray(converted_image)
converted_image = np.clip(converted_image.astype(int), 0, 255)
...
ax[1,0].imshow(converted_image[:, :, 1], cmap="Greys_r")
ax[1,1].imshow(converted_image[:, :, 2], cmap="Greys_r")
